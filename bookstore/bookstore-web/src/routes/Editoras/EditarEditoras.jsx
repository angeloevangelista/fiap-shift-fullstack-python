...
const EditarEditoras = () => {
  ...
  useEffect(() => {
    const apiEditoras = async () => {
      try {
        ...
        const response = await axios.get(`http://localhost:3000/editoras/${id}`, {
          headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.put(`http://localhost:3000/editoras/${id}`, editora, {
        headers: {
  ...
  if (error) {
    return (
      <div className="max-w-md mx-auto mt-10 p-6 bg-white rounded shadow">
        ...
        <p className="text-center mt-4">Redirecionando para lista de Editoras...</p>
      </div>
    );
  }
