...
const EditarLivros = () => {
  ...
  useEffect(() => {
    const apiLivros = async () => {
      try {
        ...
        const livroRes = await axios.get(`http://localhost:3000/livros/${id}`, {
          headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.put(
        `http://localhost:3000/livros/${id}`,livro, {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
        }
    ...
    if (error) {
      return (
        <div className="max-w-md mx-auto mt-10 p-6 bg-white rounded shadow">
          ...
          <p className="text-center mt-4">Redirecionando para a lista de livros...</p>
        </div>
      );
    }
  ...
  return (
    <div className="max-w-md mx-auto mt-10 p-6 bg-white rounded shadow">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <label className="block text-sm font-medium tex-gray-700 mb-1">ano</label>
          <input
            type="text"
            name="ano"
            value={livro.ano}
